...
class ChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        print('domain_name_from_conusmer :', self.domain_name)
    ...
    def get_chat(self):
        try:
            # chat = ChatRoom.objects.get(session=self.session_key, site=self.domain_name)
            chat = ChatRoom.objects.get(session=self.session_key)
            # if chat == None:
            #     chat =  ChatRoom.objects.get(creator=self.session_key, domain_name=self.domain_name)
            return chat
    ...
    def create_message(self, text):
        if self.user.is_staff:
            Message.objects.create(chat_room=self.chat,
                                   user=self.user,
                                   body=text,
                                   )
...
# class AdminCunsumer(AsyncWebsocketConsumer):
    ...
    # def get_chat(self):
    #     try:
    #         chat = ChatRoom.objects.get(unique_code=self.room)
    #         # if chat == None:
    #         #     chat =  ChatRoom.objects.get(creator=self.session_key, domain_name=self.domain_name)
    #         return chat
    #     except ChatRoom.DoesNotExist:
    ...
    # def create_message(self, text):
    #     # if self.user.is_staff:
    #     Message.objects.create(chat_room=self.chat.id,
    #                            user=self.user,
    #                            session=self.
    #                            body=text,
    #                            )
